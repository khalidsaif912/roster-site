(function () {
  ...
  function createChangerBtn() {
    ...
    btn.style.cssText = `
      ...
      background:rgba(20,20,24,0.60);border:1px solid rgba(255,255,255,0.20);
      color:#fff;border-radius:8px;padding:4px 7px;
      font-size:12px;cursor:pointer;backdrop-filter:blur(6px);line-height:1;
    `;
  ...
  function showBannerPicker() {
    ...
    availableBanners.forEach(name => {
      ...
      img.onerror = () => { wrap.style.display = 'none'; }; // تخفي الصورة لو ما وُجدت
  ...
  // MutationObserver عشان ينتظر الـ .header لو تأخر في الظهور
