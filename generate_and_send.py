...
# NOTE: order is 0=SUN .. 6=SAT (matches our today_dow conversion below)
...
def find_header_row(ws):
    """
    Finds a row that looks like the main header row (first cell contains employee/name marker).
    """
...
def find_employee_col(ws, start_row: int, max_scan_rows: int = 150):
    """
...
def find_days_row_near(ws, around_row: int, scan_down: int = 35):
    """
    Finds the row containing SUN..SAT near around_row.
    We consider it a days row if it contains >=3 distinct day tokens.
    """
    if around_row is None:
        around_row = 1
    start = max(1, around_row)
    end = min(ws.max_row, around_row + scan_down)

    for r in range(start, end + 1):
        vals = [norm(ws.cell(row=r, column=c).value) for c in range(1, ws.max_column + 1)]
        up_join = " | ".join(v.upper() for v in vals if v)
        hits = sum(1 for d in DAYS if d in up_join)
        if hits >= 3:
            return r, vals
    return None, None


def find_day_col_two_rows(ws, today_dow: int, today_dom: int, days_row_idx: int, days_row_vals):
    """
    Determine today's column when the structure is:
    Row A: days (SUN..SAT)
    Row B: date numbers (1..31)
    We prefer:
      - columns matching today's day-of-week token AND date number matches today_dom
    then fallback to:
      - any column where date number matches today_dom within day columns area
    """
    date_row_idx = days_row_idx + 1
    if date_row_idx > ws.max_row:
        return None

    # columns that match today's DOW token in the days row
    candidates = []
    token = DAYS[today_dow]
    for c, val in enumerate(days_row_vals, start=1):
        if token in val.upper():
            candidates.append(c)

    # Try strict: candidate col AND date matches
    for c in candidates:
        if _as_int(ws.cell(row=date_row_idx, column=c).value) == today_dom:
            return c

    # Fallback: search any day-labeled column where date matches
    day_cols = []
    for c, val in enumerate(days_row_vals, start=1):
        up = val.upper()
...
def find_day_col_single_row(ws, today_dow: int, header_row_idx: int):
    """
    Old/simple mode: day tokens are in the same header row.
    """
    header_vals = [norm(ws.cell(row=header_row_idx, column=c).value) for c in range(1, ws.max_column + 1)]
    token = DAYS[today_dow]
...
def find_day_col(ws, header_row_idx: int, today_dow: int, today_dom: int):
    """
    Master day column detector:
    1) Try two-row mode (days row + date row)
    2) If not found, try single-row token match
    """
    days_row_idx, days_row_vals = find_days_row_near(ws, header_row_idx, scan_down=45)
    if days_row_idx and days_row_vals:
        c = find_day_col_two_rows(ws, today_dow, today_dom, days_row_idx, days_row_vals)
        if c:
            return c

    # fallback: maybe header row itself contains SUN..SAT
    return find_day_col_single_row(ws, today_dow, header_row_idx)
...
def main():
    ...
    for sheet_name, dept_name in DEPARTMENTS:
        ...
        if not header_row_idx:
            all_sections_html += f"<div style='text-align:center;color:#b00020;font-weight:800;'>⚠️ لم أستطع تحديد صف العنوان (EMPLOYEE/NAME) في شيت {dept_name}</div>"
            all_sections_html += "<hr style='border:none;border-top:1px solid #eee;margin:18px 0;'>"
            continue

        day_col = find_day_col(ws, header_row_idx, today_dow, today_dom)
        if not day_col:
