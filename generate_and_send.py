...
<div class="wrap">
  ...
  <div class="header">
    <h1>📋 Duty Roster</h1>
    <button class="dateTag" id="dateTag" type="button" aria-label="Choose date">📅 {date_label}</button>
...
(function(){{
  ...
  function checkAndRedirectToToday(){{
    ...
    if(dateMatch){{
      ...
      // إذا التاريخ في الصفحة مختلف عن اليوم
      if(pageDate !== todayStr){{
        // تحقق من sessionStorage - إذا كان موجود، يعني هذا تحديث (F5)
        var isPageLoad = sessionStorage.getItem('pageLoaded');
        if(isPageLoad){{
          // تحديث الصفحة → امسح العلامة وارجع لتاريخ اليوم
          sessionStorage.removeItem('pageLoaded');
          // احصل على المسار الأساسي
          var basePath = path.replace(/\/date\/\\d{{4}}-\\d{{2}}-\\d{{2}}\\/.*$/,'/').replace(/\/now\/.*$/,'/').replace(/\/+$/,'');
          // ارجع للصفحة الرئيسية (مع أو بدون /now/)
          if(isNowPage){{
            window.location.href = basePath + '/now/';
          }} else {{
            window.location.href = basePath + '/';
          }}
          return true;
        }} else {{
          // أول تحميل بعد الضغط على التاريخ → ضع علامة
  ...
  // عند تحميل الصفحة، تحقق من التاريخ
  if(checkAndRedirectToToday()) return;

  function openPicker(e){{
    // iOS: prevent text selection + ensure tap triggers the picker
  ...
  // iOS/WhatsApp in-app browser: touchend is more reliable than click
  tag.addEventListener('click', openPicker, {{ passive:false }});
  tag.addEventListener('touchend', openPicker, {{ passive:false }});
  ...
  picker.addEventListener('change', function(){{
    ...
    // عند اختيار تاريخ جديد، امسح العلامة حتى يتم وضعها من جديد
    sessionStorage.removeItem('pageLoaded');
    ...
    // بناء المسار الجديد: /date/YYYY-MM-DD/ أو /date/YYYY-MM-DD/now/
...
def build_pretty_email_html(active_shift_key: str, now: datetime, all_shifts_by_dept: list, pages_base: str) -> str:
    """
    HTML email for: ACTIVE SHIFT only + Standby that matches the same shift.
    - active_shift_key: Morning / Afternoon / Night
    - all_shifts_by_dept: [{"dept": str, "shifts": {group: [{"name":..,"shift":..}, ...]}}]
    """

    def standby_matches_shift(active: str, shift_text: str) -> bool:
        up = norm(shift_text).upper()
        # normalize: allow emoji + spaces
        # Match explicit standby codes
        if "STAE" in up:
            return active == "Afternoon"
        if "STNE" in up:
            return active == "Night"
        if "STME" in up or "STMN" in up or "STM" in up:
            return active == "Morning"
        # Less specific: STN -> Night
        if re.search(r"\bSTN\b", up) or up.startswith("STN"):
            return active == "Night"
        # If it's just "ST"/"STANDBY" without qualifier, don't include (avoid wrong shift)
        return False

    # Collect counts and build dept blocks
    total_active = 0
    total_standby = 0
    dept_blocks = []

    for d in all_shifts_by_dept:
        dept = d.get("dept", "Department")
        shifts = d.get("shifts", {}) or {}

        active_list = list(shifts.get(active_shift_key, []) or [])
        standby_raw = list(shifts.get("Standby", []) or [])
        standby_list = [e for e in standby_raw if standby_matches_shift(active_shift_key, e.get("shift", ""))]

        if not active_list and not standby_list:
            continue
        ...
        def rows_html(items, color):
            ...
            for i, e in enumerate(items):
                ...
                rows.append(f"""
                  <tr>
                    <td style=\"padding:10px 14px;border-top:1px solid #e2e8f0;background:{bg};\">
                      <span style=\"font-size:14px;font-weight:700;color:#0f172a;\">{e['name']}</span>
                    </td>
                    <td style=\"padding:10px 14px;border-top:1px solid #e2e8f0;text-align:right;background:{bg};\">
                      <span style=\"font-size:13px;font-weight:700;color:{color};white-space:nowrap;\">{e['shift']}</span>
                    </td>
                  </tr>
                """)
            return "\n".join(rows)
        ...
        active_colors = SHIFT_COLORS.get(active_shift_key, SHIFT_COLORS["Other"])
        standby_colors = SHIFT_COLORS.get("Standby", SHIFT_COLORS["Other"])

        active_table = ""
        if active_list:
            active_table = f"""
            <table role=\"presentation\" cellpadding=\"0\" cellspacing=\"0\" style=\"width:100%;border:1px solid {active_colors['border']};border-radius:14px;overflow:hidden;margin-top:10px;\">
              <tr>
                <td colspan=\"2\" style=\"padding:10px 14px;background:{active_colors['summary_bg']};border-bottom:1px solid {active_colors['summary_border']};\">
                  <table role=\"presentation\" style=\"width:100%;\" cellpadding=\"0\" cellspacing=\"0\">
                    <tr>
                      <td style=\"padding:0;\">
                        <span style=\"font-size:18px;margin-right:8px;\">{active_colors['icon']}</span>
                        <span style=\"font-size:15px;font-weight:900;color:{active_colors['label_color']};\">{active_shift_key}</span>
                      </td>
                      <td style=\"padding:0;text-align:right;\">
                        <span style=\"display:inline-block;padding:4px 12px;border-radius:999px;background:{active_colors['count_bg']};color:{active_colors['count_color']};font-size:13px;font-weight:900;\">{len(active_list)}</span>
                      </td>
                    </tr>
                  </table>
                </td>
              </tr>
              {rows_html(active_list, active_colors['status_color'])}
            </table>
            """
        ...
        if standby_list:
            standby_table = f"""
            <table role=\"presentation\" cellpadding=\"0\" cellspacing=\"0\" style=\"width:100%;border:1px solid {standby_colors['border']};border-radius:14px;overflow:hidden;margin-top:10px;\">
              <tr>
                <td colspan=\"2\" style=\"padding:10px 14px;background:{standby_colors['summary_bg']};border-bottom:1px solid {standby_colors['summary_border']};\">
                  <table role=\"presentation\" style=\"width:100%;\" cellpadding=\"0\" cellspacing=\"0\">
                    <tr>
                      <td style=\"padding:0;\">
                        <span style=\"font-size:18px;margin-right:8px;\">{standby_colors['icon']}</span>
                        <span style=\"font-size:15px;font-weight:900;color:{standby_colors['label_color']};\">Standby ({active_shift_key})</span>
                      </td>
                      <td style=\"padding:0;text-align:right;\">
                        <span style=\"display:inline-block;padding:4px 12px;border-radius:999px;background:{standby_colors['count_bg']};color:{standby_colors['count_color']};font-size:13px;font-weight:900;\">{len(standby_list)}</span>
                      </td>
                    </tr>
                  </table>
                </td>
              </tr>
              {rows_html(standby_list, standby_colors['status_color'])}
            </table>
            """

        dept_blocks.append(f"""
        <table role=\"presentation\" cellpadding=\"0\" cellspacing=\"0\" style=\"width:100%;background:#ffffff;border:1px solid #e2e8f0;border-radius:18px;overflow:hidden;margin-top:16px;\">
          <tr>
            <td style=\"padding:14px 16px;background:#ffffff;border-bottom:1px solid #e2e8f0;\">
              <span style=\"font-size:16px;font-weight:900;color:#0f172a;\">{dept}</span>
              <span style=\"float:right;font-size:13px;font-weight:800;color:#334155;\">{len(active_list)+len(standby_list)} total</span>
            </td>
          </tr>
          <tr>
    ...
    total = total_active + total_standby

    # CTA goes to /now/ (user can switch views)
    now_url = f"{pages_base.rstrip('/')}/now/"
...
<body style=\"margin:0;background:#f1f5f9;font-family:system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif;\">
  <table role=\"presentation\" cellpadding=\"0\" cellspacing=\"0\" style=\"width:100%;background:#f1f5f9;padding:18px 0;\">
    <tr>
      <td align=\"center\">
        <table role=\"presentation\" cellpadding=\"0\" cellspacing=\"0\" style=\"width:100%;max-width:720px;background:#0b1220;border-radius:22px;overflow:hidden;\">
          <tr>
            <td style=\"padding:18px 18px 14px 18px;color:#fff;\">
              <div style=\"font-size:18px;font-weight:900;letter-spacing:-.2px;\">📋 Duty Roster</div>
              <div style=\"margin-top:6px;font-size:13px;opacity:.9;\">{date_str} · {time_str} (Muscat)</div>
              <div style=\"margin-top:10px;display:inline-block;padding:6px 12px;border-radius:999px;background:rgba(255,255,255,.14);font-size:13px;font-weight:900;\">
                {active_shift_key} shift
              </div>
            </td>
          </tr>
          <tr>
            <td style=\"padding:14px 18px;background:#ffffff;\">
              <table role=\"presentation\" cellpadding=\"0\" cellspacing=\"0\" style=\"width:100%;\">
                <tr>
                  <td style=\"padding:0 0 8px 0;color:#0f172a;font-size:14px;font-weight:800;\">
                    Total: {total} (Active {total_active} · Standby {total_standby})
                  </td>
                </tr>
              </table>
              {''.join(dept_blocks) if dept_blocks else '<div style="padding:18px;color:#334155;">No employees found for this shift.</div>'}
              <div style=\"margin-top:18px;text-align:center;\">
                <a href=\"{now_url}\" style=\"display:inline-block;padding:12px 18px;border-radius:14px;background:#1e40af;color:#fff;text-decoration:none;font-weight:900;\">Open roster</a>
              </div>
              <div style=\"margin-top:14px;text-align:center;font-size:12px;color:#64748b;\">
                Source: <strong>{get_source_name()}</strong>
              </div>
            </td>
          </tr>
        </table>
...
def main():
    ...
    # Email: ACTIVE shift only (+ matching Standby)
    subject = f"Duty Roster — {now.strftime('%d %B %Y')} — {active_group}"
