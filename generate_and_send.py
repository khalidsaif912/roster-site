...
def find_days_row_near(ws, around_row=None, scan_down=200):
    """
    نبحث عن صف يحتوي أيام الأسبوع SUN..SAT في كامل الشيت
    بدون افتراض قربه من EMPLOYEE
    """
    for r in range(1, min(ws.max_row, scan_down) + 1):
        vals = [norm(ws.cell(row=r, column=c).value) for c in range(1, ws.max_column + 1)]
        hits = sum(1 for v in vals for d in DAYS if d in v.upper())
        if hits >= 3:
            return r, vals
    return None, None
