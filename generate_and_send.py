...
.header h1 {{ margin:0; font-size:24px; font-weight:800; position:relative; z-index:1; letter-spacing:-.3px; }}
    ...
    .header .dateTag:focus {{ outline: 2px solid rgba(255,255,255,.65); outline-offset: 2px; }}
...
(function(){{
  ...
  tag.addEventListener('click', function(e){ e.preventDefault(); openPicker(); });
  // iOS: prevent text selection / callout and ensure tap works
  tag.addEventListener('touchstart', function(e){ e.preventDefault(); }, { passive:false });
  tag.addEventListener('touchend', function(e){ e.preventDefault(); openPicker(); }, { passive:false });
