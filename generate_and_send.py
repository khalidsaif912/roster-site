...
def find_header_and_day_col(ws, today_dow: int, today_dom: int):
    """
    يدعم حالتين:
    A) صف واحد فيه SUN..SAT (مثل السابق)
    B) صفين: صف أيام + الصف اللي تحته أرقام 1..31 (المطلوب عندك)
    today_dow: 0=SUN .. 6=SAT
    today_dom: رقم اليوم بالشهر 1..31
    """
    def as_int(v):
        try:
            if v is None:
    ...
    # 1) نبحث عن صف الرأس الأساسي (EMPLOYEE/STAFF/NAME/الموظف)
    print(f"[DEBUG] البحث عن صف الرأس في الشيت...")
    for i, row in enumerate(ws.iter_rows(values_only=True), start=1):
        first = norm(row[0] if row else "")
        up = first.upper()
        if i <= 20:  # طباعة أول 20 صف للتشخيص
            print(f"[DEBUG] الصف {i}: {first[:50] if first else '(فارغ)'}")
        
        if "EMPLOYEE" in up or "STAFF" in up or "NAME" in up or "الموظف" in first:
            header_row_idx = i
            header_values = [norm(x) for x in row]
            print(f"[DEBUG] تم إيجاد صف الرأس في الصف {i}: {header_values[:10]}")
            break
    
    if header_row_idx is None:
        print("[DEBUG] لم يتم إيجاد صف الرأس!")
        return None, None, None
    
    # 2) نبحث عن صف فيه SUN..SAT (غالبًا قبل/قريب من صف الموظف)
    # نجرب من بداية الشيت إلى صف الرأس + 30
    scan_start = 1
    scan_end = min(ws.max_row, header_row_idx + 30)
    days_row_idx = None
    days_row_vals = None
    
    print(f"[DEBUG] البحث عن صف الأيام من الصف {scan_start} إلى {scan_end}...")
    for r in range(scan_start, scan_end + 1):
        vals = [norm(ws.cell(row=r, column=c).value) for c in range(1, ws.max_column + 1)]
        joined = " | ".join(v.upper() for v in vals if v)
        # وجود أكثر من يوم مؤشر قوي
        hits = sum(1 for d in DAYS if d in joined)
        if hits >= 3:
            days_row_idx = r
            days_row_vals = vals
            print(f"[DEBUG] تم إيجاد صف الأيام في الصف {r} (عدد الأيام المطابقة: {hits})")
            print(f"[DEBUG] محتوى صف الأيام: {[v for v in vals if v][:15]}")
            break
    
    # لو ما لقينا صف أيام، نرجع للطريقة القديمة (صف واحد فيه SUN..SAT في نفس row)
    if days_row_idx is None:
        print("[DEBUG] لم يتم إيجاد صف أيام منفصل، محاولة الطريقة القديمة (صف واحد)...")
        day_col_idx = None
        for idx, val in enumerate(header_values):
            ...
            for dn, key in enumerate(DAYS):
                if key in up and dn == today_dow:
                    day_col_idx = idx + 1
                    print(f"[DEBUG] تم إيجاد عمود اليوم {key} في العمود {day_col_idx}")
                    break
            if day_col_idx:
                break
        
        if day_col_idx:
            print(f"[DEBUG] النتيجة النهائية: header_row={header_row_idx}, day_col={day_col_idx}")
        else:
            print("[DEBUG] لم يتم إيجاد عمود اليوم في صف الرأس!")
        
        return header_row_idx, header_values, day_col_idx
    
    # 3) نحدد "عمود اليوم" اعتمادًا على: نفس عمود اليوم في صف الأيام + تحقق رقم اليوم بالشهر بالصف اللي تحته
    # أولًا: نجيب الأعمدة المحتملة لليوم المطلوب من صف الأيام
    target_day = DAYS[today_dow]
    print(f"[DEBUG] البحث عن اليوم المطلوب: {target_day} (dow={today_dow}, dom={today_dom})")
    
    candidate_cols = []
    for c, val in enumerate(days_row_vals, start=1):
        if target_day in val.upper():
            candidate_cols.append(c)
            print(f"[DEBUG] وجدنا {target_day} في العمود {c}: '{val}'")
    
    print(f"[DEBUG] الأعمدة المحتملة لليوم {target_day}: {candidate_cols}")
    
    # الصف اللي تحت صف الأيام مباشرة (أرقام 1..31 غالبًا)
    date_row_idx = days_row_idx + 1
    if date_row_idx > ws.max_row:
        print(f"[DEBUG] صف التواريخ المتوقع ({date_row_idx}) خارج نطاق الشيت!")
        return header_row_idx, header_values, None
    
    # قراءة صف التواريخ للتشخيص
    date_row_vals = [ws.cell(row=date_row_idx, column=c).value for c in range(1, ws.max_column + 1)]
    print(f"[DEBUG] صف التواريخ (الصف {date_row_idx}): {[str(v)[:10] if v else '-' for v in date_row_vals[:20]]}")
    
    # إذا لقينا عمود اليوم (حسب اليوم) ووجدنا رقم اليوم بالشهر في نفس العمود -> ممتاز
    for c in candidate_cols:
        v = ws.cell(row=date_row_idx, column=c).value
        v_int = as_int(v)
        print(f"[DEBUG] فحص العمود {c}: القيمة = {v}, القيمة الرقمية = {v_int}, المطلوب = {today_dom}")
        if v_int == today_dom:
            print(f"[DEBUG] ✅ تطابق! العمود {c} يحتوي على اليوم المطلوب")
            return header_row_idx, header_values, c
    
    # لو ما تطابق بنفس العمود، نجرب "إيجاد رقم اليوم" ضمن أعمدة الأيام (أحيانًا يكون الجدول منزاح)
    print(f"[DEBUG] لم يتم التطابق المباشر، محاولة البحث في جميع أعمدة الأيام...")
    day_cols_all = []
    for c, val in enumerate(days_row_vals, start=1):
        up = val.upper()
        if any(d in up for d in DAYS):
            day_cols_all.append(c)
    
    print(f"[DEBUG] جميع أعمدة الأيام: {day_cols_all}")
    
    for c in day_cols_all:
        v = ws.cell(row=date_row_idx, column=c).value
        v_int = as_int(v)
        if v_int == today_dom:
            print(f"[DEBUG] ✅ تم إيجاد رقم اليوم {today_dom} في العمود {c}")
            # التحقق من أن هذا العمود يطابق اليوم الصحيح
            day_name = days_row_vals[c-1] if c-1 < len(days_row_vals) else ""
            print(f"[DEBUG] اسم اليوم في هذا العمود: {day_name}")
            return header_row_idx, header_values, c
    
    # ما قدرنا نحدد العمود
    print("[DEBUG] ❌ لم يتم إيجاد عمود اليوم المناسب!")
    return header_row_idx, header_values, None
...
def main():
    ...
    dow = now.weekday()
    today_dow = (dow + 1) % 7
    today_dom = now.day  # رقم اليوم بالشهر (1-31)
    
    print(f"[DEBUG] التاريخ الحالي: {now}")
    print(f"[DEBUG] اليوم في الأسبوع: {today_dow} ({DAYS[today_dow]})")
    print(f"[DEBUG] رقم اليوم في الشهر: {today_dom}")
    ...
    for sheet_name, dept_name in DEPARTMENTS:
        print(f"\n[DEBUG] ===== معالجة شيت: {sheet_name} ({dept_name}) =====")
        if sheet_name not in wb.sheetnames:
            print(f"[DEBUG] الشيت {sheet_name} غير موجود في الملف!")
            continue
        ws = wb[sheet_name]
        print(f"[DEBUG] أبعاد الشيت: {ws.max_row} صف × {ws.max_column} عمود")

        header_row_idx, _, day_col = find_header_and_day_col(ws, today_dow, today_dom)
