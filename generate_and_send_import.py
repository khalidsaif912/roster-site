...
def shift_bucket(code: str) -> Tuple[str, str, str, str, str]:
    ...
    if s.startswith(("ST", "SB")) and len(s) <= 4:
    ...
    if s.startswith("TR") or "TRAIN" in s:
...
// Override openDatePicker so date navigation goes to /import/YYYY-MM-DD/
// instead of Export's /date/YYYY-MM-DD/
window.openDatePicker = function() {{
  ...
  picker.onchange = function() {{
    ...
    window.location.href = base + '/' + picker.value + '/';
  }};
}};
...
<script>
  ...
  function codeToGroup(c){c=(c||'').toUpperCase().trim();if(!c||c==='O'||c==='OFF'||c==='OFFDAY'||c==='OFF DAY')return 'Off Day';if(c==='AL'||c.indexOf('ANNUAL')>=0)return 'Annual Leave';if(c==='SL'||c.indexOf('SICK')>=0)return 'Sick Leave';if(c.startsWith('TR')||c.indexOf('TRAIN')>=0)return 'Training';if((c.startsWith('ST')||c.startsWith('SB'))&&c.length<=4)return 'Standby';if(c.startsWith('MN')||c.startsWith('ME'))return 'Morning';if(c.startsWith('AN')||c.startsWith('AE'))return 'Afternoon';if(c.startsWith('NN')||c.startsWith('NE'))return 'Night';return 'Other';}
...
def month_key_from_filename(name: str) -> str | None:
    """Extract YYYY-MM key from filename like IMP_FEB_2026.xlsx or IMP_FEB_2026v.xlsx or FEBRUARY 2026."""
    if not name:
        return None
    n = name.lower()
    n = re.sub(r"[\._\-]+", " ", n)
    ...
    m = re.search(
        ...
        r"|august|aug|september|sep|sept|october|oct|november|nov|december|dec)\b\s*(\d{4})\b",
        n,
    )
    if not m:
        return None
...
def main() -> None:
    ...
    # ── 1. قراءة اسم الملف من source_name.txt (إجباري) ───────────
    # IMPORTANT: نحن لا نقرأ الشهر من داخل الشيت أبداً.
    # يجب أن يحتوي source_name.txt على اسم ملف واضح مثل: IMP_FEB_2026.xlsx
    ...
    if not incoming_key:
        raise SystemExit(
            f"❌ Cannot detect month from source_name.txt: {source_name!r}\n"
            "Please set IMPORT_SOURCE_NAME_URL to a text file that contains something like IMP_FEB_2026.xlsx"
        )

    print(f"📅 Detected month (from txt): {incoming_key}")

    # ── 2. حساب الأشهر الثلاثة بالاعتماد على اسم الملف ────────────
    # نعتبر incoming_key هو الشهر الحالي دائماً
    try:
        anchor_year, anchor_month = map(int, incoming_key.split("-"))
        anchor_start = dt.date(anchor_year, anchor_month, 1)
    except Exception:
        anchor_start = month_start(today)
        incoming_key = anchor_start.strftime("%Y-%m")
    ...
    print(f"📅 Anchor month: {curr_key} (from source_name.txt)")
    ...
    # ── 4. تنزيل ملف الإكسل دائماً (لتحديث نفس الشهر) ─────────────
    # IMPORTANT: حتى لو كان الكاش موجود، ننزل الإكسل مرة واحدة لتحديث الشهر الحالي دائماً.
    data: bytes | None = None
    try:
        data = download_excel(url)
        print("✅ Excel downloaded successfully (forced refresh)")
    except Exception as e:
        print(f"WARNING: Could not download Excel: {e}")
        print("Will attempt to use cached rosters...")

    # ── 5. تحديث كاش الشهر الحالي فقط (إذا نجح التحميل) ───────────
    if data:
        xlsx_cache = cache_dir / f"{curr_key}.xlsx"
        meta_cache = cache_dir / f"{curr_key}.meta.json"
        ...
        print(f"✅ Cached (updated): {xlsx_cache.name}")

    # ── 6. حذف صفحات التواريخ التي لا تنتمي للأشهر الثلاثة ─────────
    ...
    # ── 9. الصفحة الرئيسية (بدون 404) ─────────────────────────────
    # نعرض "اليوم" إذا كانت صفحته موجودة (شهر اليوم متوفر).
    # وإلا نعرض أول يوم متوفر من الشهر المرتكز (من اسم الملف).
    landing_date = today
    landing_html: str | None = None

    today_dir = out_root / today.strftime("%Y-%m-%d")
    if today_dir.exists() and (today_dir / "index.html").exists():
        landing_html = (today_dir / "index.html").read_text(encoding="utf-8")
    elif parsed_for_today:
        landing_html = build_duty_html(style, export_script, parsed_for_today, today, repo_base_path="/import")
        today_dir.mkdir(parents=True, exist_ok=True)
        (today_dir / "index.html").write_text(landing_html, encoding="utf-8")

    if landing_html is None:
        # fallback 1: أي يوم من الشهر الحالي — نأخذ أقرب يوم لليوم
        matches = sorted(out_root.glob(f"{curr_key}-*/index.html"))
        if matches:
            landing_date = dt.datetime.strptime(matches[0].parent.name, "%Y-%m-%d").date()
            landing_html = matches[0].read_text(encoding="utf-8")

    if landing_html is None:
        # fallback 2: أي يوم متوفر — نأخذ الأحدث (آخر تاريخ) لتجنب يناير القديم
        all_matches = sorted(out_root.glob("2[0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]/index.html"), reverse=True)
        if all_matches:
            landing_date = dt.datetime.strptime(all_matches[0].parent.name, "%Y-%m-%d").date()
            landing_html = all_matches[0].read_text(encoding="utf-8")
            print(f"⚠️ Using fallback landing page: {all_matches[0].parent.name}")

    if landing_html is None:
        # لا يوجد أي صفحة — نبني صفحة بسيطة بدلاً من الخطأ
...
<body>
  <p style="font-family:sans-serif;text-align:center;padding:40px;">جاري التوجيه...</p>
    ...
    # fallback page: أول يوم متوفر من الشهر الحالي
    fallback_dir = out_root / "fallback"
    fallback_dir.mkdir(parents=True, exist_ok=True)
    (fallback_dir / "index.html").write_text(landing_html, encoding="utf-8")

    now_dir = out_root / "now"
    now_dir.mkdir(parents=True, exist_ok=True)
    (now_dir / "index.html").write_text(landing_html, encoding="utf-8")

    # ── 10. ملفات JSON للموظفين ───────────────────────────────────
