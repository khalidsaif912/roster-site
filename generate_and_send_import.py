...
def build_duty_html(style: str, script: str, parsed: Dict[str, Any], date_obj: dt.date, repo_base_path: str, available_months: List[str] | None = None) -> str:
  ...
  <div class="summaryBar">
    <div class="summaryChip">
      ...
      <div class="chipLabel" data-en="Employees" data-ar="\u0645\u0648\u0638\u0641">Employees</div>
    </div>
    <div class="summaryChip">
      <div class="chipVal" style="color:#059669;">{dept_count}</div>
      <div class="chipLabel" data-en="Departments" data-ar="\u0623\u0642\u0633\u0627\u0645">Departments</div>
    </div>
    <a href="{{BASE}}/my-schedules/index.html" id="myScheduleBtn" class="summaryChip" style="cursor:pointer;text-decoration:none;" onclick="goToMySchedule(event)">
      ...
      <div class="chipLabel" data-en="My Schedule" data-ar="\u062c\u062f\u0648\u0644\u064a">My Schedule</div>
    </a>
  </div>
"""
    ...
    <details class="shiftCard" data-shift="{key}" style="border:1px solid {info['accent']}44; background:{info['bg']}" {'open' if key=='Afternoon' else ''}>
      <summary class="shiftSummary" style="background:{info['bg']}; border-bottom:1px solid {info['accent']}33;">
        ...
        <span class="shiftLabel" data-shift="{key}" style="color:{info['text']};">{key}</span>
...
/* ── Header ── */
.header{{background:linear-gradient(135deg,#1a56db,#1e40af);color:#fff;border-radius:16px;padding:18px 16px 16px;margin-bottom:16px;position:relative;display:flex;align-items:center;justify-content:space-between;gap:10px}}
.header-left{{display:flex;align-items:center;gap:8px;flex:0 0 auto}}
.header-center{{flex:1;text-align:center}}
.header-right{{display:flex;align-items:center;gap:8px;flex:0 0 auto}}

/* يد الترحيب والاسم */
.welcomeMsg{{font-size:13px;color:rgba(255,255,255,.9);cursor:pointer;white-space:nowrap;display:flex;align-items:center;gap:5px;font-weight:600}}
.wavingHand{{display:inline-block;font-size:16px;animation:none}}
.wavingHand.wave{{animation:wave 1.4s ease-in-out}}
@keyframes wave{{
...
.dateTag{{background:rgba(255,255,255,.15);border:1px solid rgba(255,255,255,.4);color:#fff;border-radius:10px;padding:7px 14px;cursor:pointer;font-size:14px;font-weight:700;white-space:nowrap}}
...
/* زر اللغة */
.langToggle{{background:rgba(255,255,255,.2);border:1px solid rgba(255,255,255,.4);color:#fff;border-radius:8px;padding:5px 11px;cursor:pointer;font-size:14px;font-weight:700;white-space:nowrap;flex-shrink:0}}
.langToggle:hover{{background:rgba(255,255,255,.32)}}

/* RTL support */
body.ar .header{{direction:rtl}}
body.ar .header-left{{order:3}}
body.ar .header-center{{order:2}}
body.ar .header-right{{order:1}}
...
/* رأس المناوبة أدكن من خلفية الجسم */
.shiftSummary{{display:flex;align-items:center;gap:8px;padding:10px 14px;cursor:pointer;list-style:none;border-radius:10px;filter:brightness(0.88)}}
.shiftSummary::-webkit-details-marker{{display:none}}
.shiftBody{{filter:brightness(1.04)}}
...
.empRowAlt{{background:rgba(0,0,0,.04)}}
...
@media(max-width:500px){{.summaryBar{{gap:6px}}.chipVal{{font-size:18px}}h1{{font-size:17px}}.dateTag{{font-size:13px;padding:6px 10px}}}}
  </style>
</head>
<body>
<div class="wrap">
  ...
  <div class="header">
    <!-- يسار: يد الترحيب + الاسم -->
    <div class="header-left">
      <div class="welcomeMsg" id="welcomeMsg" onclick="goToMySchedule()" title="جدولي">
        <span class="wavingHand" id="wavingHand">👋</span>
        <span id="welcomeName"></span>
      </div>
    </div>

    <!-- وسط: العنوان والتاريخ -->
    <div class="header-center">
      <h1 id="pageTitle" data-en="Import Duty Roster" data-ar="\u062c\u062f\u0648\u0644 \u0627\u0644\u0645\u0646\u0627\u0648\u0628\u0627\u062a">Import Duty Roster</h1>
      <div class="datePickerWrapper">
        <button class="dateTag" id="dateTag" onclick="openDatePicker()" type="button">📅 <span id="dateDisplay">{date_label}</span></button>
...
function _applyLang(isAr) {{
  ...
  if (title) title.textContent = isAr ? title.getAttribute('data-ar') : title.getAttribute('data-en');
  document.querySelectorAll('[data-en][data-ar]').forEach(function(el) {{
    if (el.id !== 'pageTitle') el.textContent = isAr ? el.getAttribute('data-ar') : el.getAttribute('data-en');
  }});
  var cta = document.getElementById('ctaBtn');
  if (cta) cta.textContent = isAr ? '\ud83d\udccb \u0639\u0631\u0636 \u062c\u062f\u0648\u0644 \u0627\u0644\u0645\u0646\u0627\u0648\u0628\u0627\u062a' : '\ud83d\udccb View Full Duty Roster';
  var shiftNames = {{Morning:'\u0635\u0628\u0627\u062d',Afternoon:'\u0638\u0647\u0631',Night:'\u0644\u064a\u0644','Off Day':'\u0625\u062c\u0627\u0632\u0629','Annual Leave':'\u0625\u062c\u0627\u0632\u0629 \u0633\u0646\u0648\u064a\u0629','Sick Leave':'\u0625\u062c\u0627\u0632\u0629 \u0645\u0631\u0636\u064a\u0629',Training:'\u062a\u062f\u0631\u064a\u0628',Standby:'\u0627\u0633\u062a\u0639\u062f\u0627\u062f',Other:'\u0623\u062e\u0631\u0649'}};
  document.querySelectorAll('.shiftLabel[data-shift]').forEach(function(el) {{
    el.textContent = isAr ? (shiftNames[el.getAttribute('data-shift')] || el.getAttribute('data-shift')) : el.getAttribute('data-shift');
  }});
}}

function toggleLang() {{
  var isAr = document.body.classList.toggle('ar');
...
// ── Waving hand ──────────────────────────────────────────────────
var _welcomeName = '';
function _triggerWave() {{
  var hand = document.getElementById('wavingHand');
  if (!hand) return;
...
// ── Init on load ─────────────────────────────────────────────────
(function() {{
  var isAr = localStorage.getItem('importLang') === 'ar';
  if (isAr) document.body.classList.add('ar');
  _applyLang(isAr);
  ...
  if (empId) {{
    fetch(_importBase() + '/schedules/' + encodeURIComponent(empId) + '.json')
      .then(function(r) {{ return r.ok ? r.json() : null; }})
      .then(function(d) {{
        _welcomeName = d && d.name ? d.name.split(' ')[0] : empId;
        if (nameEl) nameEl.textContent = _welcomeName;
        setTimeout(_triggerWave, 400 + Math.random() * 600);
      }})
      .catch(function() {{
        _welcomeName = empId;
        if (nameEl) nameEl.textContent = _welcomeName;
        setTimeout(_triggerWave, 400);
      }});
    // أحياناً تلوح مرة أخرى عشوائياً
    if (Math.random() > 0.45) {{
      setTimeout(_triggerWave, 5000 + Math.random() * 8000);
    }}
  }} else {{
