...
<head>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0;-webkit-tap-highlight-color:transparent}
    ...
    .search-input{width:130px;flex:0 0 130px;background:var(--glass-bg)!important;border:1px solid var(--glass-border)!important;color:var(--ink);border-radius:0 10px 10px 0!important;padding:9px 12px;padding-left:12px;height:36px;font-size:14px;font-weight:500;outline:none;transition:border-color .15s,box-shadow .15s;backdrop-filter:blur(14px) saturate(170%);-webkit-backdrop-filter:blur(14px) saturate(170%);box-shadow:var(--glass-shadow);}
    ...
    .search-btn:hover{filter:brightness(1.15);box-shadow:0 6px 22px rgba(31,111,235,.7)!important;}
    ...
    .action-ico{font-size:16px;flex:0 0 auto}
    ...
    .tbtn.primary{background:linear-gradient(135deg,#1f6feb,#388bfd);border:none;color:#fff;}
...
<footer style="text-align:center;padding:18px 16px 28px;display:flex;flex-direction:column;align-items:center;gap:4px;pointer-events:none;user-select:none;">
  <span id="footerSource" style="font-size:11px;font-weight:600;letter-spacing:.5px;color:rgba(139,148,158,.55);display:none;"></span>
...
<div class="modal" id="tipsModal" role="dialog" aria-modal="true">
  ...
  <div class="modal-sheet">
    ...
    <div class="modal-body">
      ...
      <div class="tip"><div class="tip-ico">🖼️</div><div class="tip-text" id="tip2">Image export produces a clean card with header, calendar, and stats.</div></div>
...
<script>
  // ── مسار الـ JSON (من /import/my-schedules/ → /import/schedules/) ──
  function schedulesUrl(id){ return '../schedules/' + encodeURIComponent(id) + '.json'; }

  let data=null, month=null, months=[], lang='ar', theme='dark';

  (function(){
    const sl=localStorage.getItem('importPrefLang'), st=localStorage.getItem('importPrefTheme');
  ...
  const STAT_META=[
    {k:'work',icon:'💼',c:'c-blue',label_en:'Work Days',label_ar:'أيام عمل'},
    {k:'off',icon:'🛌',c:'c-gray',label_en:'Days Off',label_ar:'أيام راحة'},
    {k:'morning',icon:'☀️',c:'c-amber',label_en:'Morning',label_ar:'صباحي'},
    {k:'afternoon',icon:'🌤️',c:'c-orange',label_en:'Afternoon',label_ar:'مسائي'},
    {k:'night',icon:'🌙',c:'c-purple',label_en:'Night',label_ar:'ليلي'},
    {k:'standby',icon:'🧍',c:'c-pink',label_en:'Standby',label_ar:'احتياطي'},
    {k:'leaves',icon:'✈️',c:'c-green',label_en:'Leaves',label_ar:'إجازات'},
  ];

  const T={
    en:{title:'Import - My Schedule',sub:'Enter your Employee ID',heroTitle:'Your Import roster in one view',heroDesc:'Enter your Employee ID to view the month, export as PDF, Image, or add to your calendar.',tipsTitle:'Help & Tips',tipsSub:'Quick help for using the roster.',langBtn:'ع',ph:'e.g. 12345',sbtn:'📅 View',loading:'Loading…',notFound:'Employee not found',notFoundSub:'ID not in the system. Please check and try again.',e1:'Search your schedule',e2:'Enter your Employee ID above to view your monthly Import roster.',pdf:'PDF',img:'Image',ics:'ICS',share:'Share',print:'Print',stats:'Stats',changeId:'Change ID',setAsMyId:'Set as My ID',days:['Sun','Mon','Tue','Wed','Thu','Fri','Sat'],months:['January','February','March','April','May','June','July','August','September','October','November','December'],saveSub:'Open your schedule faster next time.',saveYes:'✅ Save',saveNo:'Not now',idActTitle:'Employee ID',idActChange:'Change saved ID?',idActChangeSub:'This will remove the saved ID from this device.',idActPin:'Set as My ID?',idActPinSub:'Save this ID on this device for faster access.',cancel:'Cancel',ok:'OK'},
    ar:{title:'الوارد - جدولي',sub:'أدخل رقمك الوظيفي',heroTitle:'جدول الوارد في عرض واحد',heroDesc:'أدخل رقمك الوظيفي لعرض الشهر، ثم حمّل PDF/صورة أو أضفه للتقويم.',tipsTitle:'مساعدة ونصائح',tipsSub:'مساعدة سريعة لاستخدام الجدول.',langBtn:'EN',ph:'مثال: 12345',sbtn:'📅 عرض',loading:'جاري التحميل…',notFound:'لم يُعثر على موظف',notFoundSub:'الرقم غير موجود في النظام. تحقق وأعد المحاولة.',e1:'ابحث عن جدولك',e2:'أدخل رقمك الوظيفي أعلاه لعرض مناوبات الوارد.',pdf:'PDF',img:'صورة',ics:'ICS',share:'مشاركة',print:'طباعة',stats:'إحصائيات',changeId:'تغيير الرقم',setAsMyId:'احفظ كرقمي',days:['الأحد','الاثنين','الثلاثاء','الأربعاء','الخميس','الجمعة','السبت'],months:['يناير','فبراير','مارس','أبريل','مايو','يونيو','يوليو','أغسطس','سبتمبر','أكتوبر','نوفمبر','ديسمبر'],saveSub:'لفتح الجدول بسرعة في المرات القادمة.',saveYes:'✅ احفظ',saveNo:'ليس الآن',idActTitle:'الرقم الوظيفي',idActChange:'هل تريد حذف الرقم الوظيفي؟',idActChangeSub:'سيتم حذف الرقم المحفوظ من هذا الجهاز نهائياً.',idActPin:'هل تريد حفظ الرقم الوظيفي؟',idActPinSub:'سيتم حفظ هذا الرقم على هذا الجهاز لفتح الجدول بسرعة.',cancel:'إلغاء',ok:'موافق'}
  };
  ...
  function applyLangUI(){
    ...
    if(e1)e1.textContent=t('e1');if(e2)e2.textContent=t('e2');
    ...
    document.getElementById('tip1').textContent=lang==='ar'?'احفظ رقمك مرة واحدة — وسيظهر تلقائياً في المرة القادمة.':'Save your Employee ID once — it will load automatically next time.';
    document.getElementById('tip2').textContent=lang==='ar'?'حفظ الصورة ينتج بطاقة مرتبة بالتقويم والإحصائيات.':'Image export produces a clean card with calendar and stats.';
    document.getElementById('tip3').textContent=lang==='ar'?'استخدم تصدير ICS لإضافة المناوبات إلى تقويم Apple/Google/Outlook.':'Use ICS export to add shifts to Apple/Google/Outlook calendars.';
    document.getElementById('tipsOk').textContent='OK';
    const fc=document.getElementById('footerCredit');
    if(fc)fc.textContent=lang==='ar'?'تصميم: خالد الرقادي':'Design: KHALID ALRAQADI';
  ...
  function goBack(){
    ...
    else location.href=base+'/import/';
  }
  ...
  document.getElementById('searchForm').addEventListener('submit',async(e)=>{e.preventDefault();const id=document.getElementById('empId').value.trim();if(id)await loadSchedule(id);});

  async function loadSchedule(id){
    document.getElementById('area').innerHTML=`<div class="state-view"><div class="spin"></div><div class="state-title">${t('loading')}</div><div class="state-desc">${lang==='ar'?'انتظر لحظات…':'Please wait a moment…'}</div></div>`;
    try{
      const res=await fetch(schedulesUrl(id));
      if(!res.ok)throw new Error('not found');
      data=await res.json();
      // دعم كلا الصيغتين: schedules{} (الجديدة) أو days[] (القديمة)
      if(!data.schedules&&data.days){
        const mk=data.month||(new Date().getFullYear()+'-'+String(new Date().getMonth()+1).padStart(2,'0'));
        data={id:data.id,name:data.name,department:data.department,schedules:{[mk]:data.days.map(d=>({day:d.day,shift_code:d.code,shift_group:codeToGroup(d.code)}))}};
      }
      months=Object.keys(data.schedules||{}).sort();
      if(!months.length)throw new Error('empty');
      if(!localStorage.getItem('importSavedEmpId'))showSaveToast(id);
      ...
      document.getElementById('searchAvatarWrap').style.display='none';
      document.getElementById('area').innerHTML=`<div class="state-view"><span class="state-emoji">❌</span><div class="state-title">${t('notFound')}</div><div class="state-desc">${t('notFoundSub')}</div></div>`;
    }
  }

  function codeToGroup(c){c=(c||'').toUpperCase().trim();if(!c||c==='O'||c==='OFF')return 'Off Day';if(c==='AL'||c.includes('ANNUAL'))return 'Annual Leave';if(c==='SL'||c.includes('SICK'))return 'Sick Leave';if(c==='TR'||c.includes('TRAIN'))return 'Training';if(c.startsWith('ST')||c.includes('STANDBY'))return 'Standby';if(c.startsWith('MN')||c.startsWith('ME'))return 'Morning';if(c.startsWith('AN')||c.startsWith('AE'))return 'Afternoon';if(c.startsWith('NN')||c.startsWith('NE'))return 'Night';return 'Other';}

  function showSaveToast(id){const m=document.getElementById('toastMount');m.style.display='flex';m.innerHTML=`<div class="toast-card"><div class="toast-row"><div class="toast-ico">💾</div><div class="toast-text"><p><strong>${lang==='ar'?'هل تريد حفظ الرقم الوظيفي؟':'Save Employee ID?'}</strong><br>${t('saveSub')}</p></div></div><div class="toast-btns"><button class="tbtn primary" onclick="confirmSave('${id}')">${t('saveYes')}</button><button class="tbtn" onclick="dismissToast()">${t('saveNo')}</button></div></div>`;setTimeout(()=>dismissToast(),10000);}
  ...
  function openIdActionModal(kind){
    _pendingIdAction=kind;const m=document.getElementById('idActionModal');if(!m)return;
    document.getElementById('idActionTitle').textContent=t('idActTitle');
    document.getElementById('idActionIntroTitle').textContent=kind==='change'?t('idActChange'):t('idActPin');
    document.getElementById('idActionIntroDesc').textContent=kind==='change'?t('idActChangeSub'):t('idActPinSub');
    document.getElementById('idActionCancel').textContent=t('cancel');
    document.getElementById('idActionOk').textContent=t('ok');
    m.classList.add('open');
  }
  function closeIdActionModal(){const m=document.getElementById('idActionModal');if(m)m.classList.remove('open');_pendingIdAction=null;}
  function confirmIdAction(){
    if(_pendingIdAction==='change'){localStorage.removeItem('importSavedEmpId');closeIdActionModal();if(data)renderSchedule();}
    else if(_pendingIdAction==='pin'){if(data&&data.id)localStorage.setItem('importSavedEmpId',String(data.id));closeIdActionModal();if(data)renderSchedule();}
    else closeIdActionModal();
  }

  function calcStats(s){const r={work:0,off:0,morning:0,afternoon:0,night:0,standby:0,leaves:0};s.forEach(d=>{const g=d.shift_group||codeToGroup(d.shift_code||'');if(g==='Morning')r.morning++;else if(g==='Afternoon')r.afternoon++;else if(g==='Night')r.night++;else if(g==='Off Day')r.off++;else if(g==='Standby')r.standby++;else if(g==='Annual Leave'||g==='Sick Leave')r.leaves++;});r.work=r.morning+r.afternoon+r.night;return r;}
  ...
  function openStatsModal(){const el=document.getElementById('statsModalOverlay');if(el)el.style.display='flex';}
  function closeStatsModal(){const el=document.getElementById('statsModalOverlay');if(el)el.style.display='none';}
  ...
  function toggleMonthPicker(){
    const p=document.getElementById('monthPopup');if(!p)return;
    p.style.cssText='';p.classList.toggle('open');
    if(p.classList.contains('open')){requestAnimationFrame(()=>{const rect=p.getBoundingClientRect(),vw=window.innerWidth,margin=10;if(rect.right>vw-margin)p.style.transform=`translateX(-${rect.right-(vw-margin)}px)`;else if(rect.left<margin)p.style.transform=`translateX(${margin-rect.left}px)`;if(rect.bottom>window.innerHeight-margin){p.style.top='auto';p.style.bottom='calc(100% + 8px)';}});}
  }
  function closeMonthPicker(){const p=document.getElementById('monthPopup');if(p)p.classList.remove('open');}
  ...
  function renderSchedule(){
    ...
    const firstDow=new Date(yr,mo-1,1).getDay(),dim=new Date(yr,mo,0).getDate(),now=new Date();
    const mLabel=T[lang].months[mo-1];
    const popupHTML='<div class="month-popup-grid">'+months.map(m=>{const mm=parseInt(m.split('-')[1]);return '<div class="mp-item '+(m===month?'active':'')+'" onclick="jumpMonth(&quot;'+m+'&quot;);closeMonthPicker()">'+T[lang].months[mm-1]+'</div>';}).join('')+'</div>';
    const savedId=localStorage.getItem('importSavedEmpId');
    const isMyId=savedId===String(data.id);
    ...
    for(let w=0;w<6;w++){
      ...
      for(let d=0;d<7;d++){
        if(w===0&&d<firstDow){cells+=`<div class="day empty"></div>`;}
        else if(dc>dim){cells+=`<div class="day empty"></div>`;}
        else{rowHasDays=true;const dd=sched.find(s=>s.day===dc);const grp=dd?(dd.shift_group||codeToGroup(dd.shift_code||'')):'';const sc=grp?shiftClass(grp):'';const isToday=(now.getFullYear()===yr&&(now.getMonth()+1)===mo&&now.getDate()===dc);const code=dd?(dd.shift_code||(grp==='Off Day'?'OFF':grp==='Annual Leave'?'LV':grp==='Sick Leave'?'SL':grp==='Training'?'TR':grp==='Standby'?'ST':'')):'';const codeEl=code?`<span class="day-code">${esc(code)}</span>`:'';cells+=`<div class="day ${sc} ${isToday?'today':''}"><span class="dnum">${dc}</span>${codeEl}</div>`;dc++;}
      }
      if(dc>dim&&!rowHasDays)break;if(dc>dim)break;
    }
    const stats=calcStats(sched);
    const statsHTML=STAT_META.map(m=>`<div class="stat-card ${m.c}"><span class="stat-ico">${m.icon}</span><div class="stat-val">${stats[m.k]}</div><div class="stat-lbl">${lang==='ar'?m.label_ar:m.label_en}</div></div>`).join('');
    const avatarEl=document.getElementById('searchAvatar'),avatarWrap=document.getElementById('searchAvatarWrap'),changeSearchBtn=document.getElementById('searchChangeBtn');
    if(avatarEl)avatarEl.textContent=initials;if(avatarWrap)avatarWrap.style.display='flex';
    if(changeSearchBtn){if(isMyId){changeSearchBtn.textContent='✏️';changeSearchBtn.title=t('changeId');changeSearchBtn.style.display='grid';changeSearchBtn.onclick=()=>changeMyId();}else if(!savedId)changeSearchBtn.style.display='none';else{changeSearchBtn.textContent='📌';changeSearchBtn.title=t('setAsMyId');changeSearchBtn.style.display='grid';changeSearchBtn.onclick=()=>setAsMyId();}}
    document.getElementById('area').innerHTML=`
      <div id="exportArea" class="export-area">
        <div class="emp-banner">
          <div class="emp-info"><div class="emp-name-row"><div class="emp-name">${esc(data.name)}</div></div><div class="emp-dept">${esc(data.department)}</div></div>
          <div class="emp-right"><div class="month-wrap"><button class="month-picker-btn" id="mpBtn" onclick="toggleMonthPicker()">${esc(mLabel)} <span class="mpb-arrow">▼</span></button><div class="month-popup" id="monthPopup">${popupHTML}</div></div></div>
        </div>
        <div class="cal-card"><div class="cal-head">${dayHdr}</div><div class="cal-body">${cells}</div></div>
      </div>
      <div class="actions-card">
        <div class="actions-grid">
          <button class="action-btn" onclick="dlPDF()"><span class="action-ico">📄</span>PDF</button>
          <button class="action-btn" onclick="dlIMG()"><span class="action-ico">🖼️</span>${lang==='ar'?'صورة':'Image'}</button>
          <button class="action-btn" onclick="openStatsModal()"><span class="action-ico">📊</span>${lang==='ar'?'إحصائيات':'Stats'}</button>
        </div>
        <div class="actions-grid" style="margin-top:8px">
          <button class="action-btn" onclick="dlICS()"><span class="action-ico">📆</span>ICS</button>
          <button class="action-btn" onclick="shareS()"><span class="action-ico">🔗</span>${lang==='ar'?'مشاركة':'Share'}</button>
          <button class="action-btn" onclick="window.print()"><span class="action-ico">🖨️</span>${lang==='ar'?'طباعة':'Print'}</button>
        </div>
      </div>
      <div class="stats-modal-overlay" id="statsModalOverlay" onclick="closeStatsModal()" style="display:none">
        <div class="stats-modal" onclick="event.stopPropagation()">
          <div class="stats-modal-head"><span>${lang==='ar'?'الإحصائيات':'Statistics'}</span><button onclick="closeStatsModal()" class="stats-modal-close">✕</button></div>
          <div class="stats-modal-body">${statsHTML}</div>
        </div>
      </div>`;
  }

  async function captureExportCanvas(){const source=document.getElementById('exportArea')||document.querySelector('.main');if(!source)throw new Error('no export area');const clone=source.cloneNode(true);const wrap=document.createElement('div');wrap.style.cssText='position:fixed;left:-9999px;top:0;width:420px;padding:16px;box-sizing:border-box;';wrap.style.background=getComputedStyle(document.body).backgroundColor||'#0d1117';wrap.appendChild(clone);document.body.appendChild(wrap);await new Promise(requestAnimationFrame);const canvas=await html2canvas(wrap,{scale:3,backgroundColor:null,useCORS:true});document.body.removeChild(wrap);return canvas;}
  async function dlIMG(){try{const canvas=await captureExportCanvas();const a=document.createElement('a');a.download=`import-${data.id}-${month}.png`;a.href=canvas.toDataURL('image/png');a.click();}catch(e){alert(lang==='ar'?'تعذر حفظ الصورة.':'Image export failed.');}}
  async function dlPDF(){try{const canvas=await captureExportCanvas();const imgData=canvas.toDataURL('image/png');const{jsPDF}=window.jspdf;const pdf=new jsPDF({orientation:'portrait',unit:'mm',format:'a4'});const pageW=210,pageH=297,margin=10,imgW=pageW-margin*2,imgH=(canvas.height*imgW)/canvas.width;pdf.addImage(imgData,'PNG',margin,margin,imgW,imgH);let rem=imgH-(pageH-margin*2);while(rem>0){pdf.addPage();pdf.addImage(imgData,'PNG',margin,margin-(imgH-rem),imgW,imgH);rem-=(pageH-margin*2);}pdf.save(`import-${data.id}-${month}.pdf`);}catch(e){alert(lang==='ar'?'تعذر حفظ PDF.':'PDF export failed.');}}
  function dlICS(){const sched=data.schedules[month]||[];const[yr,mo]=month.split('-').map(Number);const pad=n=>String(n).padStart(2,'0');const lines=['BEGIN:VCALENDAR','VERSION:2.0','PRODID:-//ImportMySchedule//EN','CALSCALE:GREGORIAN'];sched.forEach(d=>{const dt=`${yr}${pad(mo)}${pad(d.day)}`;const dn=new Date(yr,mo-1,d.day+1);const dtE=`${dn.getFullYear()}${pad(dn.getMonth()+1)}${pad(dn.getDate())}`;lines.push('BEGIN:VEVENT',`DTSTART;VALUE=DATE:${dt}`,`DTEND;VALUE=DATE:${dtE}`,`SUMMARY:${d.shift_code||d.shift_group}`,`UID:${dt}-${data.id}@import`,'END:VEVENT');});lines.push('END:VCALENDAR');const blob=new Blob([lines.join('\r\n')],{type:'text/calendar'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`import-${data.id}-${month}.ics`;a.click();URL.revokeObjectURL(a.href);}
  function shareS(){const base=location.pathname.includes('/roster-site/')?'/roster-site':'';const url=`${location.origin}${base}/import/my-schedules/?emp=${encodeURIComponent(data.id)}`;if(navigator.share)navigator.share({title:'Import - My Schedule',text:`Schedule: ${data.name}`,url});else{navigator.clipboard.writeText(url);alert(lang==='ar'?'تم نسخ الرابط ✅':'Link copied ✅');}}

  /* Icon swap */
  (function(){const cal=document.getElementById('brandCal'),q=document.getElementById('brandQ');if(!cal||!q)return;let sc=true;setInterval(()=>{sc=!sc;cal.classList.toggle('show',sc);q.classList.toggle('show',!sc);},2600);})();

  /* Aurora scroll */
  (function(){const topbar=document.querySelector('.topbar'),aI=document.getElementById('auroraBarInner'),aB=document.getElementById('auroraBar');if(!topbar)return;function sp(){const h=topbar.offsetHeight||62;if(aB)aB.style.top=h+'px';}window.addEventListener('scroll',()=>{if(aI)aI.classList.toggle('visible',window.scrollY>5);},{passive:true});setTimeout(sp,100);window.addEventListener('resize',sp);})();

  /* Init */
  applyLangUI();
  const p=new URLSearchParams(location.search).get('emp');
  if(p){document.getElementById('empId').value=p;loadSchedule(p);}
  else{const saved=localStorage.getItem('importSavedEmpId');if(saved){document.getElementById('empId').value=saved;loadSchedule(saved);}}
</script>
</body>
</html>
