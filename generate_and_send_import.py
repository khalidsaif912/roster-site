...
def download_excel(url: str) -> bytes:
    """Download Excel from OneDrive/SharePoint share link, handling HTML preview redirects."""
    if not url:
        raise ValueError("IMPORT_EXCEL_URL is empty")

    # Convert SharePoint/OneDrive share links to direct download URLs
    try:
        ...
        if "sharepoint.com" in host or "onedrive.live.com" in host or "1drv.ms" in host:
            path = u.path or ""
            # SharePoint share link pattern: /:x:/g/personal/.../XXXX?e=YYYY
            # Convert to direct download via _layouts/15/download.aspx
            if "/:x:/" in path or "/:w:/" in path or "/:f:/" in path:
                # Encode the share URL as base64 for the download endpoint
                encoded = _b64.b64encode(url.encode()).decode().rstrip("=")
                # Use SharePoint's internal download redirect
                base_url = f"{u.scheme}://{u.netloc}"
                url = f"{base_url}/_layouts/15/download.aspx?share={encoded}"
            else:
                # Regular URL — just add download=1
    ...
    if not data.startswith(b"PK"):
        ...
        if "text/html" in ctype:
            hint = " (got HTML preview — check OneDrive link is publicly shared and use ?download=1)"
        elif ctype.startswith("image/"):
            hint = " (got an image — IMPORT_EXCEL_URL points to wrong file)"
        raise ValueError(
            f"Downloaded file is not a valid .xlsx (Content-Type: {ctype or 'unknown'}){hint}"
        )
    return data
...
def build_duty_html(style: str, script: str, parsed: Dict[str, Any], date_obj: dt.date, repo_base_path: str) -> str:
    ...
    min_date = add_months(_month_first, -1).strftime("%Y-%m-01")
    max_date = add_months(_month_first, 1).strftime("%Y-%m-28")
...
def main() -> None:
    ...
    if source_name_url:
        try:
            r = requests.get(source_name_url, timeout=15)
            r.raise_for_status()
            source_name = r.text.strip()
        except Exception:
            pass
    ...
    # ── 5. حساب الأشهر الثلاثة ──────────────────────────────────
    prev_start = month_start(add_months(today, -1))
    curr_start = month_start(today)
    next_start = month_start(add_months(today, +1))
    ...
    for month_bytes, month_key, _ in [
    ...
    if today_dir.exists() and (today_dir / "index.html").exists():
        ...
        # بناء صفحة اليوم من البيانات المتوفرة
        duty_today = build_duty_html(style, export_script, parsed_for_today, today, repo_base_path="/import")
        today_dir.mkdir(parents=True, exist_ok=True)
        (today_dir / "index.html").write_text(duty_today, encoding="utf-8")
    else:
        raise SystemExit("❌ No data available for today's page and no cache found.")
