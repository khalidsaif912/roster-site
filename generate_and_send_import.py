...
def main() -> None:
    ...
    if data:
        ...
        try:
            ...
            # نتتبع هل استخدمنا incoming_key لشيت غامض — لتجنب الكتابة فوق كاش بشيت ثانٍ مجهول
            filename_fallback_used = False

            for sn in xls_check.sheet_names:
                sn_key = month_key_from_filename(sn)  # استخرج من اسم الشيت
                used_filename_fallback = False

                if not sn_key:
                    if incoming_key and not filename_fallback_used:
                        # الشيت اسمه غامض، نستخدم اسم الملف مرة واحدة فقط
                        sn_key = incoming_key
                        used_filename_fallback = True
                        print(f"  ℹ️  Sheet '{sn}': اسم غامض — سيُكاش باسم الملف ({incoming_key})")
                    elif incoming_key and filename_fallback_used:
                        # شيت ثانٍ بدون اسم شهر — لا يمكن تحديد شهره تلقائياً
                        print(f"  ⚠️  Sheet '{sn}': شيت إضافي بدون اسم شهر — تخطي")
                        print(f"       💡 سمِّ الشيتات بأسماء الأشهر مثل 'March 2026' لحل هذا")
                        continue
                    else:
                        print(f"  ⚠️  Sheet '{sn}': لا يمكن تحديد الشهر من الشيت ولا من اسم الملف — تخطي")
                        continue
