...
// الأشهر المتوفرة فعلاً (مولَّدة من Python)
var _availableMonths = {available_months_js};

// ── 1. إعادة التوجيه لتاريخ اليوم عند تحميل الصفحة ──────────────
(function() {{
  var pageDateStr = '{date_iso}'; // تاريخ الصفحة الحالية
  var now = new Date();
  ...
  // إذا الصفحة ليست لتاريخ اليوم → redirect لتاريخ اليوم
  if (pageDateStr !== todayStr) {{
    ...
    if (_availableMonths.length === 0 || _availableMonths.indexOf(todayMonth) !== -1) {{
      // شهر اليوم متوفر → اذهب لتاريخ اليوم
      var base = _importBase();
      var todayPath = base + '/' + todayStr + '/';
      fetch(todayPath + 'index.html', {{method: 'HEAD'}})
        .then(function(r) {{
          if (r.ok) window.location.replace(todayPath);
        }})
        .catch(function() {{}});
    }}
  }}
...
// ── 2. فتح مناوبة الوقت الحالي تلقائياً ─────────────────────────
(function() {{
  var pageDateStr = '{date_iso}';
  var now = new Date();
  var y = now.getFullYear();
  ...
  // فقط إذا كانت الصفحة لتاريخ اليوم
  if (pageDateStr !== todayStr) return;

  var hour = now.getHours();
  // الصباح: 06:00 - 13:59 | الظهر: 14:00 - 21:59 | الليل: 22:00 - 05:59
  var currentShift;
  if (hour >= 6 && hour < 14) {{
  ...
  // أغلق كل المناوبات أولاً ثم افتح المناوبة الحالية فقط
  setTimeout(function() {{
    var allDetails = document.querySelectorAll('details.shiftCard');
    allDetails.forEach(function(el) {{
      el.removeAttribute('open');
    }});
    var targetDetails = document.querySelectorAll('details.shiftCard[data-shift="' + currentShift + '"]');
    targetDetails.forEach(function(el) {{
      el.setAttribute('open', '');
    }});
    console.log('⏰ Auto-opened shift: ' + currentShift + ' (hour=' + hour + ')');
  }}, 100);
}})();
...
window.openDatePicker = function() {{
  ...
  picker.onchange = function() {{
    ...
    var chosenMonth = picker.value.substring(0, 7);
    if (_availableMonths.length > 0 && _availableMonths.indexOf(chosenMonth) === -1) {{
      alert('⚠️ هذا الشهر غير متوفر حالياً.\nالأشهر المتوفرة: ' + _availableMonths.join(', '));
      return;
    }}
    var base = _importBase();
