...
def main() -> None:
    ...
    # ── 3. استخراج الشهر — من اسم الملف دائماً (الأولوية لاسم الملف) ────
    # الأولوية: 1) source_name.txt  2) اسم الملف في الـ URL  3) تاريخ اليوم
    # لا نقرأ من اسم الشيت لأنه قد يكون خاطئاً (شيت يناير داخل ملف فبراير)
    incoming_key = month_key_from_filename(source_name) if source_name else None

    if not incoming_key and url:
        url_filename = url.split("/")[-1].split("?")[0]
        incoming_key = month_key_from_filename(url_filename)
        if incoming_key:
            source_name = source_name or url_filename
            print(f"📅 Detected month from URL filename: '{url_filename}' → {incoming_key}")

    if not incoming_key:
        print("⚠️ Could not detect month from filename — will use today's month")
    ...
    if landing_html is None:
        # fallback: pick first generated day in anchor month (curr_key)
    ...
    if landing_html is None:
        raise SystemExit("❌ No data available to build landing page (no cache for anchor/prev/next months).")

    (out_root / "index.html").write_text(landing_html, encoding="utf-8")
