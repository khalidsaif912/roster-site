...
def build_duty_html(style: str, script: str, parsed: Dict[str, Any], date_obj: dt.date, repo_base_path: str, available_months: List[str] | None = None) -> str:
  ...
  <div class="summary-bar">
    ...
    <div class="bar-btns">
      <button class="bar-btn" id="toggleBtn" onclick="goToMySchedule()" style="font-size:22px">👋</button>
...
:root{{
  ...
  --ink:#a8bbd8;--muted:#6a7e9c;--dim:#3d4f63;
...
/* Lang + Theme + Schedule buttons inside summary bar */
.bar-btns{{display:flex;gap:6px;flex:0 0 auto;align-self:stretch}}
.bar-btn{{
  height:100%;min-height:52px;width:46px;
  background:rgba(255,255,255,.055);border:1px solid var(--border2);
...
/* Header wave animation — continuous gentle wave */
@keyframes waveAnim{{0%,60%,100%{{transform:rotate(0)}}10%{{transform:rotate(20deg)}}20%{{transform:rotate(-10deg)}}30%{{transform:rotate(18deg)}}40%{{transform:rotate(-8deg)}}50%{{transform:rotate(12deg)}}}}
.hdr-wave{{
  display:inline-block;transform-origin:70% 80%;
  animation:waveAnim 2.4s ease-in-out 0.8s both;
}}
/* Toggle btn flip */
...
@keyframes flipIn{{0%{{opacity:0;transform:rotateY(-90deg)}}50%{{opacity:1;transform:rotateY(0)}}100%{{opacity:1;transform:rotateY(0)}}}}
.toggle-btn-flip{{animation:flipOut .35s ease forwards}}
.toggle-btn-unflip{{animation:flipIn .35s ease forwards}}
...
.emp-name{{font-weight:700;flex:1;color:#c8d8f0}}.emp-code{{font-weight:800;font-family:'DM Mono',monospace;font-size:12px;letter-spacing:.5px;color:#6b9fd4}}
  ...
  <div class="header">
    ...
    <div class="hdr-row" style="justify-content:center">
      <div class="hdr-center" style="flex:1;text-align:center">
        <div id="hdrWelcome" style="font-size:13px;font-weight:600;color:rgba(255,255,255,.65);margin-bottom:6px;min-height:20px;letter-spacing:.3px;display:flex;align-items:center;justify-content:center;gap:6px"><span class="hdr-wave" id="hdrWave"></span><span id="hdrGreet"></span></div>
...
(function(){{
  ...
  if(!btn)return;
  var states=['\U0001F44B','\U0001F4C5'];
  var idx=0;
  function flip(){{
    btn.classList.remove('toggle-btn-unflip');btn.classList.add('toggle-btn-flip');
    setTimeout(function(){{
      idx=(idx+1)%states.length;btn.textContent=states[idx];
      btn.classList.remove('toggle-btn-flip');btn.classList.add('toggle-btn-unflip');
    }},180);
  }}
...
(function(){{
  ...
  if(empId&&hdrGreet){{
    fetch(_importBase()+'/schedules/'+encodeURIComponent(empId)+'.json').then(function(r){{return r.ok?r.json():null;}}).then(function(d){{
      ...
      if(hdrWave)hdrWave.textContent='\U0001F44B';
      if(hdrGreet)hdrGreet.textContent='\u0645\u0631\u062d\u0628\u0627\u064b \u060c\u00a0'+first;
    }}).catch(function(){{if(hdrWave)hdrWave.textContent='\U0001F44B';if(hdrGreet)hdrGreet.textContent='\u0645\u0631\u062d\u0628\u0627\u064b';}});
  }}
...
<script>
  ...
  function _buildICS(){
    var sched=data.schedules[month]||[];
    ...
    var lines=[
      'BEGIN:VCALENDAR',
      'VERSION:2.0',
      'PRODID:-//ImportMySchedule//App//EN',
      'CALSCALE:GREGORIAN',
      'METHOD:PUBLISH'
    ];
    sched.forEach(function(d){
      var dt2=yr+''+pad(mo)+''+pad(d.day);
      var dn=new Date(yr,mo-1,d.day+1);
      var dtE=dn.getFullYear()+''+pad(dn.getMonth()+1)+''+pad(dn.getDate());
      var summary=(d.shift_code||d.shift_group||'').replace(/[\r\n,;\\\\]/g,' ');
      lines.push(
        'BEGIN:VEVENT',
        'UID:'+dt2+'-'+data.id+'@importschedule',
        'DTSTAMP:'+dtstamp,
  ...
  function dlICS(){
    var icsContent=_buildICS();
    var filename='import-'+data.id+'-'+month+'.ics';
    // iOS: data URI يفتح التقويم مباشرة
    ...
    // Android: share sheet يسمح باختيار التقويم
    if(navigator.canShare&&navigator.canShare({files:[new File([blob],filename,{type:'text/calendar'})]})){
      navigator.share({files:[new File([blob],filename,{type:'text/calendar'})],title:filename}).catch(function(){_dlICSBlob(blob,filename);});
      return;
    }
    _dlICSBlob(blob,filename);
  }
  function _dlICSBlob(blob,filename){
  ...
  function dlICSToCalendar(app){
    var icsContent=_buildICS();
    var filename='import-'+data.id+'-'+month+'.ics';
    var blob=new Blob([icsContent],{type:'text/calendar;charset=utf-8'});
    var file=new File([blob],filename,{type:'text/calendar'});
    if(app==='google'){
      // فتح Google Calendar مباشرة
  ...
  function showCalendarPicker(){
    var modal=document.getElementById('calPickerModal');
    if(modal){modal.classList.add('open');return;}
    var overlay=document.createElement('div');
    overlay.id='calPickerModal';
    overlay.style.cssText='position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,.7);backdrop-filter:blur(6px);display:flex;align-items:flex-end;justify-content:center;padding:0;animation:fadeIn .15s ease;';
    overlay.innerHTML='<div style="width:100%;max-width:480px;background:var(--surface);border-radius:24px 24px 0 0;padding:20px 20px calc(20px + env(safe-area-inset-bottom));border:1px solid var(--border2);">'+
      '<div style="width:40px;height:4px;background:var(--dim);border-radius:2px;margin:0 auto 18px;opacity:.5"></div>'+
      '<div style="font-size:16px;font-weight:700;color:var(--ink);margin-bottom:4px;text-align:center;">'+(lang==='ar'?'أضف إلى التقويم':'Add to Calendar')+'</div>'+
      '<div style="font-size:13px;color:var(--muted);text-align:center;margin-bottom:20px;">'+(lang==='ar'?'اختر التطبيق':'Choose app')+'</div>'+
      '<div style="display:flex;flex-direction:column;gap:10px;">'+
        '<button onclick="dlICS();closeCalPicker();" style="display:flex;align-items:center;gap:14px;padding:14px 18px;background:var(--surface2);border:1px solid var(--border2);border-radius:14px;color:var(--ink);font-size:15px;font-weight:600;cursor:pointer;width:100%;">'+
          '<span style="font-size:24px;">📆</span>'+
          '<span>'+(lang==='ar'?'فتح بـ... (اختر التطبيق)':'Open with... (choose app)')+'</span>'+
        '</button>'+
        '<button onclick="_dlICSBlob(new Blob([_buildICS()],{type:'text/calendar'}),'import-'+data.id+'-'+month+'.ics');closeCalPicker();" style="display:flex;align-items:center;gap:14px;padding:14px 18px;background:var(--surface2);border:1px solid var(--border2);border-radius:14px;color:var(--ink);font-size:15px;font-weight:600;cursor:pointer;width:100%;">'+
          '<span style="font-size:24px;">💾</span>'+
          '<span>'+(lang==='ar'?'تحميل الملف فقط':'Download file only')+'</span>'+
        '</button>'+
      '</div>'+
      '<button onclick="closeCalPicker()" style="margin-top:14px;width:100%;padding:12px;background:transparent;border:1px solid var(--border);border-radius:12px;color:var(--muted);font-size:14px;cursor:pointer;">'+(lang==='ar'?'إلغاء':'Cancel')+'</button>'+
    '</div>';
    overlay.addEventListener('click',function(e){if(e.target===overlay)closeCalPicker();});
    document.body.appendChild(overlay);
  }
  function closeCalPicker(){
    var m=document.getElementById('calPickerModal');
    if(m)m.remove();
  }
