...
def _sharepoint_to_download_url(url: str) -> str:
    """
    Convert any SharePoint/OneDrive share link to a direct download URL.
    Uses Microsoft's OneDrive Sharing API with base64-encoded share URL.
    Works for /:x:/p/, /:t:/p/, /:x:/g/ and similar personal share links.
    """
    ...
    if "sharepoint.com" in host or "onedrive.live.com" in host or "1drv.ms" in host:
        # Microsoft's method: base64-encode the full share URL
...
def download_excel(url: str) -> bytes:
    """Download Excel from OneDrive/SharePoint share link."""
    if not url:
        raise ValueError("IMPORT_EXCEL_URL is empty")

    dl_url = _sharepoint_to_download_url(url)
    ...
    if not data.startswith(b"PK"):
        ...
        if "text/html" in ctype:
            hint = " (got HTML — check that the OneDrive link is shared as 'Anyone with the link')"
        raise ValueError(
...
def download_text(url: str) -> str:
    """Download a small text file from OneDrive/SharePoint share link."""
    if not url:
        return ""
    dl_url = _sharepoint_to_download_url(url)
