...
(function() {{
  ...
  // Welcome message — نعرض الاسم من JSON بدل الرقم
  ...
  if (welcome && empId) {{
    ...
    welcome.textContent = (isAr ? '\u0645\u0631\u062d\u0628\u0627\u064b \u2022 ' : 'Welcome \u2022 ') + empId;
    var schedBase = _importBase() + '/schedules/' + encodeURIComponent(empId) + '.json';
    fetch(schedBase)
      .then(function(r) {{ return r.ok ? r.json() : null; }})
      .then(function(d) {{
        if (d && d.name) {{
          var firstName = d.name.split(' ')[0];
          welcome.textContent = (isAr ? '\u0645\u0631\u062d\u0628\u0627\u064b\u060c ' : 'Welcome, ') + firstName + ' \ud83d\udc4b';
        }}
      }})
      .catch(function() {{}});
  }}
...
def main() -> None:
    ...
    for month_bytes, month_key, month_start_date in [
        ...
        if not cache_valid and data and incoming_key == month_key:
            ...
            if stale_cache.exists(): stale_cache.unlink(); print(f"  🗑️  حذف كاش ملوث: {month_key}.xlsx")
