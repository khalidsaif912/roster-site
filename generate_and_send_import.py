    ...
    <div class="hdr-row">
      <div class="hdr-left">
        <div class="welcome-wrap" onclick="goToMySchedule()" title="\u062c\u062f\u0648\u0644\u064a">
          <span class="wave-hand" id="waveHand">\U0001F44B</span>
...
(function(){{
  ...
  var nameEl=document.getElementById('welcomeName'),handEl=document.getElementById('waveHand');
  if(empId&&nameEl){{fetch(_importBase()+'/schedules/'+encodeURIComponent(empId)+'.json').then(function(r){{return r.ok?r.json():null;}}).then(function(d){{nameEl.textContent=d&&d.name?d.name.split(' ')[0]:empId;setTimeout(_wave,500+Math.random()*400);if(Math.random()>.4)setTimeout(_wave,5e3+Math.random()*7e3);}}).catch(function(){{nameEl.textContent=empId;setTimeout(_wave,500);}});}}
  else{{if(handEl)handEl.style.display='none';if(nameEl)nameEl.style.display='none';}}
