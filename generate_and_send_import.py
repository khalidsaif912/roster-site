...
def download_excel(url: str) -> bytes:
    ...
    # Force direct download for SharePoint/OneDrive share links
    if "sharepoint.com" in url or "1drv.ms" in url or "onedrive.live.com" in url:
        # Add download=1 to force direct download instead of HTML preview
        sep = "&" if "?" in url else "?"
    ...
    if not data.startswith(b"PK"):
        raise ValueError(
            f"Downloaded content is not a valid XLSX file (got {len(data)} bytes, "
            f"starts with: {data[:40]!r}). "
            "Check if the OneDrive link requires ?download=1 or is publicly shared."
        )
    return data
